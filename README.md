...
**Version 1.7.7-rc1 | September 2026**
...
ICS Sim Platform is a software-based industrial control system simulator that generates realistic process data across eight major industry verticals. It serves data over the same protocols used by PLCs, RTUs, and edge gateways — Modbus TCP, OPC-UA, and MQTT/SparkplugB — so SCADA, historian, and analytics clients connect to it the same way they connect to field devices.

The platform runs as a single self-contained executable on Windows, Linux, or macOS. No additional hardware, virtual machines, or supporting infrastructure is required: configure it, run it, and connect your client tools.

---

## When to Use It

Testing industrial software requires live process data. ICS Sim Platform supplies that data in place of:

- **A physical test lab** — limited to the process types the lab was built for
- **A vendor PLC simulator** — requires matching hardware or licensed software
- **Static data files** — no protocol interaction, no write-back, no live behavior
- **A production system** — access-restricted, and changes carry operational risk

It provides live, interactive, protocol-correct process simulation that starts from a command line on any workstation.
...
**Software Vendors** — Test your ICS/OT software against multi-industry data: oil & gas, water, power, building automation, and manufacturing subsystems from one instance. Run ICS Poller alongside the simulator to measure polling bandwidth at a given device count and poll interval.

**Cybersecurity Teams** — Build OT security lab environments with live protocol traffic. Test IDS rules, practice network forensics, and run tabletop exercises against live Modbus, OPC-UA, and MQTT traffic. Inject cyberattack scenarios (setpoint manipulation, sensor spoofing, alarm suppression) and check whether your tooling detects them.
...
38 subsystem definitions organized across eight industry verticals. Each subsystem models the complete register map of a real-world field device, so the number and type of tags vary from subsystem to subsystem — as few as 72 tags and as many as 136, depending on the subsystems chosen. Every tag has a realistic value range, engineering units, and inter-tag dependencies. The exact register map and tag counts for any subsystem are in its `Register_Map_<CODE>.md` document.
...
ICS Sim Platform presents as a Modbus TCP server supporting all standard function codes (FC01 through FC16). Connect any Modbus master — Autosol ACM/eACM, Ignition, FactoryTalk, Geo SCADA, or a custom driver — and poll registers exactly as you would a real PLC. The dashboard can generate ready-to-import tag definition files for Autosol eACM and Ignition SCADA, so tags do not have to be entered by hand.
...
The platform exposes a fully browsable OPC-UA address space organized by subsystem and device. Tags appear with correct data types, engineering units, and descriptive metadata.
...
The platform publishes process data as a SparkplugB edge node — the same topology used by Ignition MQTT Engine, Cirrus Link Chariot, and AWS IoT SiteWise.
...
Combined with MQTT report-by-exception filtering, SparkplugB metric aliasing, and NDATA node-level publishing (as an alternative to the default per-device DDATA mode), the full optimization stack reduces MQTT bandwidth by 80-95% compared to unconditional polling. Production edge gateways use the same techniques to reduce traffic over constrained cellular and satellite links.

---

## AI/ML Testbed Capabilities

The platform includes an AI/ML pipeline for training and deploying OT-specialized language models. AI features require a Professional or Enterprise license (demo mode includes a 10-minute trial).
...
Every step — from simulated telemetry to a deployed fine-tuned model — runs on this platform. See `AI_Testbed_Guide.md` for the step-by-step procedure.
...
**Goal:** Validate an Ignition tag configuration for a gas gathering station with six compressor units before going on-site.
...
**Goal:** Simulate non-standard equipment that doesn't match any of the 38 built-in subsystems — for example, a pump skid and a metering skid with specific tag layouts.
...
**Goal:** Verify that an OPC-UA client handles 20 devices across four subsystem types with correct data types and subscriptions.

Run `ICSSimPlatform.exe --system "WTP 5, DIST 5, LIFT 5, WWTP 5" --opcua` and connect your client to port 4840. Browse the address space, subscribe to value changes, write setpoints, and verify your driver handles Float, Boolean, and Integer tag types correctly across 2,310 tags.

### Cybersecurity Training Lab

**Goal:** Supply live Modbus traffic for a multi-day ICS security course, for students to analyze in Wireshark and write Snort rules against.
...
**Goal:** Build an on-premises LLM that detects compressor valve degradation and cyberattacks in OT telemetry.
...
**Goal:** Measure how much bandwidth SparkplugB report-by-exception saves over a satellite link compared with flat Modbus polling.
...
**Goal:** Evaluate a data diode at the OT edge against realistic ICS traffic volumes and protocol diversity.

Run `ICSSimPlatform.exe --system "NGGP 10, SUBX 3, WTP 5" --mqtt` to generate live Modbus, OPC-UA, and MQTT traffic across 18 devices and 1,966 tags. Place your data diode between the simulation network and your monitoring infrastructure and validate one-way data flow, protocol translation, and throughput under sustained load.

### Historian Backfill Testing

**Goal:** Verify that a historian correctly handles store-and-forward backfill when a remote site reconnects after a network outage.
...
**Goal:** Validate translation accuracy in a gateway that converts between Modbus, OPC-UA, and MQTT, with all three protocols serving the same data.
